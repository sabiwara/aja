defmodule Aja.Enum.PropTest do
  ...
  property "Aja.Enum functions should return the same as mirrored Enum functions" do
    check all(list <- list_of(value()), i1 <- integer(), i2 <- integer()) do
      ...
      assert slice_2 === Aja.Enum.slice(list, i1..i2)
      assert slice_2 === Aja.Enum.slice(vector, i1..i2)
      assert slice_2 === Aja.Enum.slice(stream, i1..i2)
      assert Enum.slice(map_set, i1..i2) === Aja.Enum.slice(map_set, i1..i2)
      ...
      assert capture_error(Enum.join(map_set, ",")) === capture_error(Aja.Enum.join(map_set, ","))
