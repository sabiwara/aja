...
- Add `A.Enum.concat/1` and `A.Enum.into/3`
...
- `A.Enum` as a faster `Enum` module optimized for Aja structures (vectors, ord
  maps)
- Added `A.ord_size/1` macro
- `A.vec/1` can pattern-match on first and last elements
- `A.ord/1` warns on duplicate errors and can generate the AST on compile time
  for constant keys
...
- Add `A.Vector.take_while/2`, `A.Vector.drop_while/2` and
  `A.Vector.split_while/2`
...
- Add A.Vector.each/2
- Add `A.Vector.slice/2`, `A.Vector.slice/3`, `A.Vector.take/2` and
  `A.Vector.drop/2`
...
- Invoke callbacks in the right order for: `A.Vector.map/2`,
  `A.Vector.map_intersperse/3`, A.Vector.map_join/3, A.Vector.any?/2,
  A.Vector.all?/2
...
- A.Vector.sum/1 adds using the same order as `Enum.sum/1`, avoiding slight
...
- Changed signature of `A.OrdMap.foldl/3`, `A.OrdMap.foldr/3`, A.RBMap.foldl/3,
  A.RBMap.foldr/3
